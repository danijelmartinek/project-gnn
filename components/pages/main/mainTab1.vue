            ...
            <v-card class="elevation-0 data-table">
              ...
              >
                <template slot="items" slot-scope="props">
                  ...
                  <td class="text-xs-center prop-font-size">{{ props.item.dayDistance }} m</td>
                </template>
...
export default {
  ...
  methods: {
    getData(){
      axios.get(`http://localhost:3000/api/activities/dailydata`)
      .then(response => {
      if(response.data[0] !== 'noData'){
        var i = 1;
        var top3 = [];
        response.data.sort(function(a, b){
            return b.dayDistance-a.dayDistance;
        })
