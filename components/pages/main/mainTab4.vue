<template>
  <v-container class="pa-0 container" fluid>
            ...
            <v-card class="elevation-0 data-table">
              ...
              >
                <template slot="items" slot-scope="props">
                  ...
                  <td class="text-xs-center prop-font-size">{{ props.item.yearDistance }} m</td>
                </template>
    ...
    <v-layout v-else>
      <h1 class="no_data">Nema zapisa</h1>
    </v-layout>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    getData(){
      axios.get(`http://localhost:3000/api/activities/yearlydata`)
      .then(response => {
      if(response.data[0] !== 'noData'){
        var i = 1;
        var top3 = [];
        response.data.sort(function(a, b){
            return b.yearDistance-a.yearDistance;
        })
